...
public class BltManager {
    ...
    public static BluetoothSocket getmBluetoothSocket() {
    ...
    public void registerBltReceiver(Context context, OnRegisterBltReceiver onRegisterBltReceiver) {
        ...
        intent.addAction(BluetoothAdapter.ACTION_DISCOVERY_FINISHED);  //注册扫描结束
    ...
    public void unregisterReceiver(Context context) {
        ...
        if (getmBluetoothAdapter() != null)
            getmBluetoothAdapter().cancelDiscovery();//停止搜索
    }
...
//            }
            ...
            //状态改变时  远程设备改变时
                ...
                //扫描结束
                MainActivity.blt_status_text.setText("搜索结束");
                MainActivity.btl_bar.setVisibility(View.GONE);
            }
        }
    };
     ...
     */
    public  void getBltList(Handler handler,String i) {
     ...
     * @return
     */
    private  void connect(BluetoothDevice btDev, Handler handler,String i) {
        try {
            //通过和服务器协商的uuid来进行连接
            ...
            if (!getmBluetoothSocket().isConnected()) {
                ...
                getmBluetoothSocket().connect();//有时候连接不上蓝牙 可能是远程蓝牙设备有问题
            }
            ...
            handler.sendEmptyMessage(1);  //自动连接失败 就提示手动连接
            Log.d("blueTooth", "连接失败");
            try {
     ...
     * @param btDev
     */
    public  void createBond(BluetoothDevice btDev, Handler handler,String i) {
